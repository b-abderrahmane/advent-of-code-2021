...
fn evaluate_instruction(current_position: Vec<i32>, instruction: &Vec<String>) -> Vec<i32>{
    let mut position = current_position;
    if instruction.len() == 2 {
        match &instruction[0][..] {
            "forward" => {
                position[0] = position[0] + instruction[1].parse::<i32>().unwrap();
                position[1] = position[1] + position[2]*instruction[1].parse::<i32>().unwrap();
            },
            "up" => {
                position[2] = position[2] - instruction[1].parse::<i32>().unwrap()
            },
            "down" => {
                position[2] = position[2] + instruction[1].parse::<i32>().unwrap()
            },
...
fn main() {
    ...
    let mut current_position = vec![0, 0, 0];
